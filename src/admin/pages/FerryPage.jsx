    ...
    <div className="space-y-4">
      ...
      <FormField label="Route Name">
        <Input
          ...
          placeholder="Kolkata - Howrah Ferry"
        />
      </FormField>
      <div className="grid grid-cols-2 gap-4">
...
export default function FerryPage() {
  ...
  const handleSave = () => {
    ...
    if (modal === "add") {
      dispatch({ type: "ADD_FERRY", payload });
      toast("Ferry route added");
    } else {
      dispatch({ type: "UPDATE_FERRY", payload });
      toast("Ferry route updated");
    }
    setModal(null);
  };
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">
            {state.ferryRoutes.length} routes
          </p>
        </div>
          ...
          actions={(row) => (
            <>
              ...
              <ActionBtn
                variant="ghost"
                onClick={() => {
                  dispatch({ type: "DELETE_FERRY", payload: row.id });
